...
extension UIView {
    ...
    func setCornerAndShaow(shadowRadius: CGFloat = 1, shadowOpacity: Float = 0.1, contentSize: CGSize) {
        setNeedsDisplay()
        layoutIfNeeded()
        ...
        let path = UIBezierPath.init()
        path.move(to: .init(x: -shadowRadius, y: -shadowRadius/2.0))
        path.addLine(to: .init(x: contentSize.width + shadowRadius, y: -shadowRadius/2.0))
        path.addLine(to: .init(x: contentSize.width + shadowRadius, y: contentSize.height + shadowRadius * 4))
        path.addLine(to: .init(x: -shadowRadius, y: contentSize.height + shadowRadius * 4))
        path.addLine(to: .init(x: -shadowRadius, y: -shadowRadius))
